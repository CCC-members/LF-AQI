...
% In this case there will be single BrainStorm Protocol
...
path='/data3_260T/data/CCLAB_DATASETS/CHBM/CHBM_ARIOSKY/run/BrainStorm_Output/Protocols/';
%% list all the folder in the given path.
% In this case we will get the list of all the brainstrom protocols
bs_prot=dir(path);
protocol_names={bs_prot(1:length(bs_prot)).name};
%% Clear the first two names as it have null value
for i=length(protocol_names):-1:1
    tmp=protocol_names{i};
    if strcmp((tmp(1)),'.') || strcmp((tmp(1)),'i')
        protocol_names(i)=[];     % delete files starting with '.'
        bs_prot(i)=[];
    end
end
...
for i=length(protocol_names):-1:1
    tmp=protocol_names{i};
    protocol_anat(i)=strcat(path,'/', protocol_names(i),'/anat');
    protocol_data(i)=strcat(path,'/', protocol_names(i),'/data');
end
%%  For each protocol 
for iProtocol=1:length(protocol_names)
%get all the subject for iProtocol
subject_names=dir(char(protocol_anat(iProtocol)));
...
%% FOR EACH SUBJECT
 for iSubject=1:length(subject_filenames)
     iTotal=iTotal+1;
     SubjectAnat = strcat(protocol_anat(iProtocol),'/',subject_filenames{iSubject});
     SubjectData= strcat(protocol_data(iProtocol),'/',subject_filenames{iSubject},'/@raw',subject_filenames{iSubject});
     %%load Data
